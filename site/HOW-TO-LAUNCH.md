...
- **Editing text later:** all copy is in `sections.jsx` and `editorial.jsx`. The current design is typographic (no photos); spare imagery still lives in `assets/imagery/` if ever needed.
- **Performance note:** this build compiles in the browser, which adds about a second on first load — fine for a one-page site. If ARCJ wants a faster, fully production-grade build (precompiled), that's the "Route B" rebuild.
